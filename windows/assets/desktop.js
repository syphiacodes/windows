...
time.innerHTML = new Intl.DateTimeFormat('de-CH', { hour: 'numeric', minute: 'numeric', hour12: true }).format(new Date())
time.setAttribute("title", new Intl.DateTimeFormat('de-CH', { hour: 'numeric', minute: 'numeric', hour12: true, year: 'numeric', month: 'long', day: 'numeric' }).format(new Date()))
...
document.querySelector(".cassnet").ondblclick = function () {
    setTimeout( ()=> { this.classList.remove("selected") }, 2 )
    window.open("https://www.cass.li/")
}

document.querySelector(".my-computer").ondblclick = function () {
    setTimeout( ()=> { this.classList.remove("selected") }, 2 )
    window.open("https://syphia.li")
}
...
document.querySelector("textarea").value = "cheeky little test website\nhope you enjoy clicking on the links\nfeel free to delete the text and type whatever you want (within reason)\ndrag the tab around if you really want"
